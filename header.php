...
<div id="page" class="site">
	...
	<header id="masthead" class="site-header">
		...
		<div class="site-branding mx-12 lg:mx-20 md:28 py-5 flex">
			<div class="w-20 h-20 mr-3 rounded-full"><?php the_custom_logo(); ?></div>
			<div class="grid place-content-center">
				<?php
				if ( is_front_page() && is_home() ) :
					?>
					<h1 class="site-title font-serif text-lg sm:text-2xl m-0"><a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home"><?php bloginfo( 'name' ); ?></a></h1>
					<?php
				else :
					?>
					<p class="site-title font-serif text-lg sm:text-2xl m-0"><a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home"><?php bloginfo( 'name' ); ?></a></p>
					<?php
				endif;
				$cmsassignmenttheme_description = get_bloginfo( 'description', 'display' );
				if ( $cmsassignmenttheme_description || is_customize_preview() ) :
					?>
					<p class="site-description m-0 text-xs sm:text-base text-gray-400"><?php echo $cmsassignmenttheme_description; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
				<?php endif; ?>
			</div>
		</div><!-- .site-branding -->
